...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char* method, *requestTarget, *httpVersion;
            
            // create array to hold request-line substrings
            char* requestLine[3] = {NULL, NULL, NULL};
    
            // check METHOD
            requestLine[0] = strtok(line, " ");
            ...
            // check that request-target doesn't contain quotes
            ...
            // point to beginning of query in request-target
            char* query_ptr = strchr(requestTarget, '?');
            ...
            // create temporary request-target to manipulate, keeping original request-target intact
            char request_ptr[strlen(requestTarget)];
            strcpy(request_ptr, requestTarget);
            
            // extract absolute path from temporary request-target
            char* absolutePath = strtok(request_ptr, "?"); 
    
            // copy root into path, concat absolute path to end of root
            ...
            // point to beginning of extension in absolute path
            char* extension_ptr = strchr(absolutePath, '.');

            // point behind '.'
            char* after_period = &extension_ptr[1];
     
            // extract
            char extension[strlen(extension_ptr)];
            strcpy(extension, after_period);
...
const char* lookup(const char* extension)
{
    if (extension != NULL)
    {
        ...
        char new_extension[n + 1];

        for (int i = 0; i < n; i++)
            new_extension[i] = tolower(extension[i]);
    
        // add null-terminating character
        new_extension[n] = '\0';

        // return MIME type depending on extension   
        if (strcmp("css", new_extension) == 0)
            return "text/css";
    
        if (strcmp("html", new_extension) == 0)
            return "text/html";
        
        if (strcmp("gif", new_extension) == 0)
            return "image/gif";
        
        if (strcmp("ico", new_extension) == 0)
            return "image/x-icon";
            
        if (strcmp("jpg", new_extension) == 0)
            return "image/jpeg";
        
        if (strcmp("js", new_extension) == 0)
            return "text/javascript";
        
        if (strcmp("png", new_extension) == 0)
            return "image/png";
    }
